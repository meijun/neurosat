...
class NeuroSAT(object):
    ...
    def validate(self, valid_data_dir):
        valid_problems_loader = init_problems_loader(valid_data_dir)
        results = []

        while valid_problems_loader.has_next():
            valid_problems, valid_filename = valid_problems_loader.get_next()

            epoch_valid_cost = 0.0
            epoch_valid_mat = ConfusionMatrix()

            for problem in valid_problems:
                d = self.build_feed_dict(problem)
                logits, cost = self.sess.run([self.logits, self.cost], feed_dict=d)
                epoch_valid_cost += cost
                epoch_valid_mat.update(problem.is_sat, logits > 0)

            epoch_valid_cost /= len(valid_problems)
            epoch_valid_mat = epoch_valid_mat.get_percentages()

            results.append((valid_filename, epoch_valid_cost, epoch_valid_mat))
